...
def SIGN_2P_B(conn, addr):
    while True:
        # 接收p1
        p1 = conn.recv(1024).decode()
        conn.sendall("I have get P1[0]".encode())
        p2 = conn.recv(1024).decode()
        conn.sendall("I have get P1[1]".encode())
        p1 = int(p1)
        p2 = int(p2)
        P1 = (p1, p2)
        ...
        Pub = Mul_Add(P1[0], P1[1], gmpy2.invert(d2, n))
        P = Add(Pub[0], Pub[1], Gx, p -  Gy)

        # 接收e和Q1
        e = conn.recv(1024).decode()
        conn.sendall("I have get e".encode())
        q1 = conn.recv(1024).decode()
        conn.sendall("I have get Q1[0]".encode())
        q2 = conn.recv(1024).decode()
        conn.sendall("I have get Q1[1]".encode())
        ...
        Q3_TEMP = Mul_Add(Q1[0], Q1[1], k3)
        Q3 = Add(Q3_TEMP[0], Q3_TEMP[1], Q2[0], Q2[1])
        r = (Q3[0] + int(e, 16)) % n
        s2 = (d2 * k3) % n
        s3 = (d2 * (r + k2)) % n
...
mthread = Thread(target = SIGN_2P_B, args = (conn, addr))
mthread.start()
sock.close()